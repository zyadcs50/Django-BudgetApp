...
from .forms import UserRegisterForm, ExpenseForm, BudgetCycleForm
...
class BudgetContext:
    def __init__(self, user, session):
        ...
        if budget:
            ...
            if 'safe_daily_limit' not in session:
                safe_daily_limit = round(budget.total_amount / remaining_days, 2)  # ✅ FIX
                session['safe_daily_limit'] = float(safe_daily_limit)
                print(f"IN IF condition - Recalculated safe_daily_limit: {safe_daily_limit}")
            elif day_changed:
                safe_daily_limit = round(remaining_balance / remaining_days, 2)  # ✅ FIX
                session['safe_daily_limit'] = float(safe_daily_limit)
                print(f"Day changed - Recalculated safe_daily_limit: {safe_daily_limit}")
            else:
